...
void UStateBase::InitializeThisState(ASampleCharacter * Character)
{
	...
	DefaultLink.NextState = Character->GetDefaultState();
...
void UStateBase::CheckAllStateLinks(ASampleCharacter * Character, const TArray<FInputFrame> &InputStream)//, TArray<FStateLink> StateLinks)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("%s @CheckStateLinks ReferenceObject was null"), *GetName());
		return;
	}

	if (Character->QuedState != nullptr)
	{
		...
		//(InputlessSharedStateLinks && InputlessSharedStateLinks->StateLinks.Num()))
	{
		...
		if (InputStream.Num())
		{
			if (DoesLastElementOfInputstreamContainAcitveButtons(InputStream))
			{
				...
				// SET LINKS
				if (CheckStateLinks(Character, InputStream, StateLinks)) return;
	...
	else
	{
		...
		QueueState(Character, Character->GetDefaultState(), DefaultLink);
		return;
	}
}

bool UStateBase::CheckStateLinks(ASampleCharacter * Character, const TArray<FInputFrame> & InputStream, TArray<FStateLink> StateLinksToCheck)
{
	auto retflag = false;
	if (StateLinksToCheck.Num())
	{
		for (int32 i = 0; i < StateLinksToCheck.Num(); i++)
		{
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("%s @CheckStateLinks LINK %d =i was not a valid index in StateLinks"), *GetName(), i);
				return retflag;
			}

			if (!StateLinksToCheck[i].NextState)
			{
				UE_LOG(LogTemp, Warning, TEXT("%s @CheckStateLinks LINK %d had no next State"), *GetName(), i);
				return retflag;
			}


			switch (CheckOneStateLink(Character, InputStream, StateLinksToCheck[i]))
			{
			case EStateMachineCompletionType::Accepted:
				...
				Cast<ASamplePlayerController>(Character->GetController())->ClearInputStream();
				UE_LOG(LogTemp, Warning, TEXT("@CheckStateLinks %s LINK Accepted"), *StateName.ToString());
				QueueState(Character, StateToSwitchTo, StateLinksToCheck[i]);
				retflag = true;
				return retflag;
				break;
...
EStateMachineCompletionType UStateBase::CheckOneStateLink(ASampleCharacter * Character, const TArray<FInputFrame> &InputStream, FStateLink OneStateLink)
{
	...
	if (amountOfButtonBitflags > 0 && OneStateLink.RequieredDirections.Num())
	{
		...
		CorrectDirectionInputs = FindRequiredDirectionsInInputStream(OneStateLink, InputStream, Character, true);
	...
	{
		CorrectDirectionInputs = FindRequiredDirectionsInInputStream(OneStateLink, InputStream, Character, false);
...
int32 UStateBase::FindRequiredDirectionsInInputStream(FStateLink &OneStateLink, const TArray<FInputFrame> & InputStream, ASampleCharacter * Character, bool allowButtons)
{
...
bool UStateBase::FoundRequiredDirectionIndexInInputStream(
...
bool UStateBase::IsButtonInFrameJustPressed(const FInputFrame & InputFrame)
{
	bool retflag = false;
...
int32 UStateBase::SetFoundCorrectDirectional(int32 CorrectDirectionalInputs, int32 Index, TArray<FLinkConditonDirection> &TempRequierdDirections)
{
	TempRequierdDirections[Index].FoundThisDirInput = true;
	return CorrectDirectionalInputs += 1;
}

int32 UStateBase::FindRequiredButtonsInInputStream(const TArray<FInputFrame> & InputStream, FStateLink &OneStateLink)
{
...
/*
bool UStateBase::CheckSpecialLinkCondition(FStateLink &OneStateLink, ASampleCharacter * Character)
{
...
*/
bool UStateBase::DoesLastElementOfInputstreamContainAcitveButtons(const TArray<FInputFrame> & InputStream)
{
...
//----------------------------------QUEUE STATE---------------------------------------------------------------------------
void UStateBase::QueueState(ASampleCharacter * Character, UStateBase * DestiantionState, FStateLink OneStateLink)
{
	...
	{
		//TODO change to player pawn base class when done
		if (Character)
		{
			const ASamplePlayerController* debug = Cast<ASamplePlayerController>(Character->GetController());
			UE_LOG(LogTemp, Warning, TEXT("%f @QueueState finished"), debug->DebuggTimer);
			UE_LOG(LogTemp, Warning, TEXT("%s @QueueState successful cast"), *GetName());
			...
			{
				Character->SetCurrentState(DestiantionState);
			}
			else
			{
				Character->QuedState = DestiantionState;
			}

		}
		else
		{
			UE_LOG(LogTemp, Warning, TEXT("%s @QueueState did not manage to cast BBCharacter"), *GetName());
			return;
		}
...
int32 UStateBase::CheckHowManyBitFlagsSet(int32 FlagsToCheck, int32 EnumCount)
{
	int32 ReturnValue = 0;
